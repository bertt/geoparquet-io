...
- **`gpio check spatial` no longer reports optimally Hilbert-sorted files as poorly ordered.** The verdict came from the fraction of *consecutive* row-group pairs whose bboxes overlap, which is ~1.00 for a perfectly ordered file — Hilbert-sorted row groups are spatially adjacent by construction, so their boxes touch. It is now the estimated row-group skip rate as a fraction of what the file's row-group count allows, so it is meaningful at every count (a two-row-group file can never skip more than ~50%; 589 groups should skip ~98%). Below five row groups the verdict is withheld rather than guessed — measured across Hilbert-sorted samples, a perfectly sorted file scores as low as 0.11 at two row groups. Locality metrics are computed for every file rather than only suspect ones, the failure message quotes the deciding numbers instead of the overlap ratio, and `--query-fraction`, `--num-samples`, `--seed` and `--min-efficiency` tune the estimate. The 0.7 default is placed from measurement: across 206 files with five or more row groups from every catalog in the Portolan registry, efficiencies run 0.000 for one unsorted file, a cluster of eight under-sorted files from 0.534 to 0.699, then nothing until 0.722 and a median of 0.979 — and every file in that cluster reached 0.87–0.97 after a re-sort. ([#755](https://github.com/geoparquet/geoparquet-io/issues/755))
